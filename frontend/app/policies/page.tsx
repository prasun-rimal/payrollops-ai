...
export default function PoliciesPage() {
  ...
  return <section className="content page-content">
    <div className="page-actions"><div><strong>{policies.length} policy sections indexed</strong><span>{user?.role === "admin" ? "Each section receives an embedding for country-aware retrieval." : "Policy creation is restricted to operations administrators."}</span></div>{user?.role === "admin" && <button className="primary" onClick={() => setOpen(true)}><Plus size={17} />Add policy section</button>}</div>
