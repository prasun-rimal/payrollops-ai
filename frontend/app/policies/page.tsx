...
export default function PoliciesPage() {
  ...
  return <section className="content page-content">
    ...
    {open && <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-heading"><div><h2>Add policy section</h2><p>New text becomes available to the retrieval workflow.</p></div><button type="button" className="icon-button" onClick={() => setOpen(false)}><X size={18} /></button></div>{error && <div className="error-banner">{error}</div>}<label>Document name<input name="document_name" required minLength={3} placeholder="Global Payroll Controls" /></label><div className="form-grid"><label>Section<input name="section" required minLength={3} placeholder="Payment currency" /></label><label>Country<input name="country" required defaultValue="Global" /></label></div><label>Policy text<textarea name="content" required minLength={20} rows={6} placeholder="Describe the control and required operator action." /></label><div className="modal-actions"><button type="button" className="secondary" onClick={() => setOpen(false)}>Cancel</button><button className="primary" type="submit">Index policy</button></div></form></div>}
  </section>;
}
